...
for t in range(1, time_steps):
    if 2000<t and 2050>=t:
        S[t,0,0]+=(np.sum(S[t-1,:,:])+np.sum(SQ[t-1,:,:]))*0.03
    accumulated_dna_health=accumulated_dna_health*(1-r_dm(t))
    current_time = t * dt
...
def sen_ration(A,B):
    ...
    return ratios
S_ratios=sen_ration(stem1_S+stem1_SQ,stem1_SN)
D_ratios=sen_ration(stem1_D,stem1_DN)
# Step 5: Plot the scatter plot

plt.plot(range(len(S_ratios)), S_ratios, marker='o', label='stem cells', color='red', markersize=0, linewidth=0.5)
plt.plot(range(len(D_ratios)), D_ratios, marker='s', label='differentiated cells', color='blue', markersize=0, linewidth=0.5)
...
for t in range(1, time_steps):
    if 2000<t and 2050>=t:
        for a in range(1, age_steps):
            S[t-1,a,0]=0.8*np.sum(S[t-1,a,:])
            S[t-1,a,1:]=0.2*S[t-1,a,1:]
            D[t-1,a,0]=0.8*np.sum(D[t-1,a,:])
            D[t-1,a,1:]=0.2*D[t-1,a,1:]
            SQ[t-1,a,0]=0.8*np.sum(SQ[t-1,a,:])
            SQ[t-1,a,1:]=0.2*SQ[t-1,a,1:]
    accumulated_dna_health=accumulated_dna_health*(1-r_dm(t))
    current_time = t * dt
...
def sen_ration(A,B):
    ...
    ratios = sum_B / (sum_A + epsilon)  # Element-wise division
    return ratios
S_ratios=sen_ration(seno_S+seno_SQ,seno_SN)
D_ratios=sen_ration(seno_D,seno_DN)
# Step 5: Plot the scatter plot
...
stem_D_ratios=sen_ration(stem1_D,stem1_DN)
telo_D_ratios=sen_ration(stem_D,stem_DN)
seno_D_ratios=sen_ration(seno_D,seno_DN)
base_D_ratios=sen_ration(base_D,base_DN)
# Step 5: Plot the scatter plot


plt.plot(range(len(stem_D_ratios)), stem_D_ratios, marker='s', label='Stem cell therapy', color='blue', markersize=0, linewidth=0.7)

plt.plot(range(len(telo_D_ratios)), telo_D_ratios, marker='s', label='Telomere lengthening', color='purple', markersize=0, linewidth=0.7)

plt.plot(range(len(seno_D_ratios)), seno_D_ratios, marker='s', label='Senolytics', color='green', markersize=0, linewidth=0.7)

plt.plot(range(len(base_D_ratios)), base_D_ratios, marker='s', label='Control', color='red', markersize=0, linewidth=0.7)
...
def r_a_dn(t,f_c):
    ...
    a_by_damage=(1- (1-k_a_dn)** unrepaired_damage)
    return max(0, (a_by_damage + k_c*(1-f_c) )*resistance_to_apoptosis+r_immuno_surv*accumulated_dna_health)
# Generate values for t return max(0, (a_by_damage + k_c*(1-f_c) )*resistance_to_apoptosis+r_immuno_surv*accumulated_dna_health)
...
telo_stem_D_ratios=sen_ration(telo_stem_D,telo_stem_DN)
seno_telo_D_ratios=sen_ration(seno_telo_D,seno_telo_DN)
seno_stem_D_ratios=sen_ration(seno_stem_D,seno_stem_DN)
all_D_ratios=sen_ration(all_D,all_DN)
# Step 5: Plot the scatter plot
...
plt.plot(range(len(telo_stem_D_ratios)), telo_stem_D_ratios, marker='s', label='Telomere lengthening+Stem cell therapy', color='green', markersize=0, linewidth=0.7)

#plt.plot(range(len(seno_telo_D_ratios)), seno_telo_D_ratios, marker='s', label='seno_telo_D_ratios', color='orange', markersize=0, linewidth=0.7)

#plt.plot(range(len(seno_stem_D_ratios)), seno_stem_D_ratios, marker='s', label='seno_stem_D_ratios', color='pink', markersize=0, linewidth=0.7)

#plt.plot(range(len(all_D_ratios)), all_D_ratios, marker='s', label='All', color='green', markersize=0, linewidth=0.7)
...
for t in range(1, time_steps):
    if 2000<t and 2050>=t:
        for a in range(1, age_steps):
            S[t-1,a,0]=0.5*np.sum(S[t-1,a,:])
            S[t-1,a,1:]=0.5*S[t-1,a,1:]
            D[t-1,a,0]=0.5*np.sum(D[t-1,a,:])
            D[t-1,a,1:]=0.5*D[t-1,a,1:]
            SQ[t-1,a,0]=0.5*np.sum(SQ[t-1,a,:])
            SQ[t-1,a,1:]=0.5*SQ[t-1,a,1:]
    accumulated_dna_health=accumulated_dna_health*(1-r_dm(t))
    current_time = t * dt
